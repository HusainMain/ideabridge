...
function isSingleTokenGibberish(token) {
    ...
    if (letterLen >= 8 && vowel < 0.15)
        return true;
    if (letterLen >= 10 && vowel < 0.22)
        return true;
    if (/[bcdfghjklmnpqrstvwxyz]{6,}/i.test(lower))
        return true;
    return false;
}
function looksLikeKeyboardSmash(value) {
    const trimmed = value.trim();
    if (trimmed.length < 6)
        return false;
...
function lettersOnlyLength(value) {
...
function hasLowInformation(value) {
    const trimmed = value.trim();
...
function fieldsAreIdentical(fields) {
...
function checkField(value, label) {
    ...
    if (isNumbersOnly(value))
        issues.push(`${label} contains only numbers`);
    if (isPunctuationOnly(value))
        issues.push(`${label} contains only punctuation or symbols`);
    if (hasRepeatedCharacters(value))
        issues.push(`${label} has excessive repeated characters`);
    if (hasRepeatedWords(value))
        issues.push(`${label} repeats the same word`);
    if (looksLikeKeyboardSmash(value))
        issues.push(`${label} looks like random keyboard input`);
    if (isMostlySymbols(value))
        issues.push(`${label} is mostly symbols`);
    if (hasLowInformation(value))
        issues.push(`${label} has too little meaningful information`);
    return issues;
}
export function runDeterministicValidation(fields) {
    const missing = [];
    const suggestions = [];
    const fieldIssues = [];
...
function deriveMissingFromIssues(fields, issues) {
    const missing = [];
    for (const { key, label } of TEXT_FIELDS) {
        const value = fields[key];
